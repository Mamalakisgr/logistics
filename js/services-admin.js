document.addEventListener("DOMContentLoaded", function() {
  ...
  serviceForm.addEventListener("submit", function(event) {
    ...
    const endpoint = isUpdating ? `/api/service-categories/${isUpdating}` : "/api/service-categories";
    ...
    .then(response => response.json())
    .then(data => {
      console.log(isUpdating ? "Service category updated:" : "Service category added:", data);
...
function loadCategoryToForm(id) {
  ...
  .then(category => {
      ...
      document.getElementById("serviceName").value = category.services[0].serviceName;
      document.getElementById("description").value = category.services[0].description;
      document.getElementById("service-form").setAttribute('data-update-id', category._id);
  });
}
...
detailForm.addEventListener("submit", function(event) {
    ...
    const method = isUpdating ? "PUT" : "POST";
    const endpoint = isUpdating ? `/api/service-details/${isUpdating}` : "/api/service-details";
    ...
    .then(response => response.json())
    .then(data => {
        console.log(isUpdating ? "Detailed service updated:" : "Detailed service added:", data);
