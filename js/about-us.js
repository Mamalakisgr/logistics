...
async function deleteEmployee(employeeId) {
  try {
    const response = await fetch(
      `${API_BASE_URL}/delete-employee/${employeeId}`,
      {
        method: "DELETE",
      }
    );
  ...
  try {
    ...
    Object.keys(storeGroups).forEach((store) => {
      ...
      storeGroups[store].forEach((employee) => {
        ...
        pTitle.textContent = employee.title
          ? employee.title.gr
          : "Unknown Title";
...
async function updateEmployee(e) {
  ...
  formData.append(
    "title[gr]",
    document.getElementById("update-title-gr").value
  );
  formData.append(
    "title[en]",
    document.getElementById("update-title-en").value
  );
  ...
  try {
    const response = await fetch(
      `${API_BASE_URL}/update-employee/${employeeId}`,
      {
        method: "PUT",
